...
def reconcile_ohlc_levels(
    ...
    *,
    ...
    A daily close must lie inside that day's low/high range. When a vendor returns an
    impossible close but the open/high/low bar remains coherent, the close is reconstructed
    by time-weighted log interpolation between the nearest valid surrounding closes and then
    projected into the observed bar. A second pass repairs a reversible one-row scale pulse
    only when the interpolated level is independently supported by the same day's OHLC range.

    Inversion is performed on all OHLC fields before validation; high and low swap under a
    reciprocal quote. This keeps direct and inverse FX representations economically identical.
    """
    ...
    close_in_bar = _inside_bar(levels, high, low)
    trusted = bar_valid & close_in_bar

    for position in range(len(levels)):
        if trusted.iloc[position]:
            continue
        if not bar_valid.iloc[position]:
            continue
    ...
    # Some feeds provide a formally wide bar that still contains a close expressed in the
    # wrong temporary unit. A reversible scale pulse is corrected only when the geometric
    # bridge from the previous to next close falls inside the current day's valid OHLC bar.
...
def _valid_bar(open_: pd.Series, high: pd.Series, low: pd.Series) -> pd.Series:
    ...
    ordered = high >= low
    open_inside = _inside_bar(open_, high, low)
    return positive & ordered & open_inside
